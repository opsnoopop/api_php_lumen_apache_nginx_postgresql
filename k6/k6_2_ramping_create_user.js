...
export default function () {
  ...
  let res = http.post("http://container_nginx:80/users", JSON.stringify(body), headers);
...
export function handleSummary(data) {
  ...
  const filename = "/k6/2_ramping_create_user_" + year +  month + day + "_" + hours + minutes + seconds + ".html";
  
  return {
    [filename]: htmlReport(data, {
      title: "2_ramping_create_user_api_php_lumen_apache_nginx_postgresql_" + year + month + day + "_" + hours + minutes + seconds
    }),
  };
}